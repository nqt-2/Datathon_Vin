...
| :--- | :--- | :--- |
| **Nguyễn Quốc Thái** | Team Leader | Thiết lập GitHub, Giải quyết câu hỏi trắc nghiệm, Phân tích EDA, Xây dựng & Tối ưu Mô hình dự báo (Phần 3). |
| **Nguyễn Đình Sơn** | Coding Specialist | Giải quyết câu hỏi trắc nghiệm, Phân tích EDA & Trực quan hóa dữ liệu (Phần 2), Xây dựng & Tối ưu Mô hình dự báo (Phần 3). |
| **Nguyễn Phúc Duy Khang** | Business Analyst | Giải quyết câu hỏi trắc nghiệm, Phân tích EDA, Phân tích Insight kinh doanh (Prescriptive Analysis), Viết báo cáo PDF theo chuẩn NeurIPS. |

## 3. Cấu trúc Repository
Repository được tổ chức khoa học để Ban giám khảo dễ dàng theo dõi và tái lập kết quả:

* `**/data**`: Chứa 14 file dữ liệu CSV gốc tải từ Kaggle (products, orders, customers,...).
...
* `**Datathốn Report.pdf**`: Báo cáo chi tiết tóm tắt các phát hiện chính và phương pháp tiếp cận kỹ thuật.
* `**requirements.txt**`: File để cài phiên bản của các thư viện cần thiết cho venv.

## 4. Pipeline thực hiện
Quy trình làm việc của nhóm tuân thủ các giai đoạn chuẩn trong Khoa học dữ liệu:

1.  **Làm sạch dữ liệu:** Xử lý giá trị thiếu (null), chuẩn hóa kiểu dữ liệu và định dạng ngày tháng cho 14 bảng dữ liệu.
...
3.  **Xây dựng Mô hình:** Thực hiện Feature Engineering (tạo biến lag, mùa vụ) và áp dụng Cross-validation theo chuỗi thời gian để dự báo cột Revenue và COGS. Sử dụng mô hình XGBoost với RandomSearchCV để tìm tham số tối ưu cho mô hình.
4.  **Giải thích mô hình:** Sử dụng Feature Importance để xác định các yếu tố ảnh hưởng lớn nhất đến doanh thu.
...
1. Clone repository này về máy cá nhân bằng lệnh:  
   `git clone https://github.com/nqt-2/Datathon_Vin`
